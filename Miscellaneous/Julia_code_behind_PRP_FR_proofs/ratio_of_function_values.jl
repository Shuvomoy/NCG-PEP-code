...
function feasible_sol_generator_Lyapunov_Ada_PEP(μ, L, R, η_2_input;
...
ι_f = 1, ι_x = 1, ι_g = 1, ι_p = 1
	...
	for i in k-1:k-1
		...
		# generate β[i]
		β[i] = ((g_array[:,i+1]'*g_array[:,i+1])- η_2_input*(g_array[:,i+1]'*g_array[:,i]))/(g_array[:,i]'*g_array[:,i])
		# generate p[i+1]
...
# R = 1
# η_2_input = 1
# G_feas, Ft_feas, L_cholesky_feas, γ_kM1_feas, β_kM1_feas = feasible_sol_generator_Lyapunov_Ada_PEP(μ, L, R, η_2_input; c_f = 1, c_x = 1, c_g = 1, c_p = 1,
# ι_f = 1, ι_x = 1, ι_g = 1, ι_p = 1)


# A funciton for generating contraction factor of FR

function FR_contraction_factor_gen(c_0_input_initial, μ, L)

    # $\epsilon_k=\sqrt{\nicefrac{(1-\cond)^2 (k-1)^2}{4 \cond + (1-\cond)^2 (k-1)^2}}

    q = μ/L

    δ = c_0_input_initial # 1 + (1 - q + 2*sqrt((-1 + c_0_input_initial)*q))^2/(4*q)
...
function Ratio_of_Function_Values_Model(
  ...
  idx_set_λ_ws_effective, G_ws, Ft_ws, L_cholesky_ws, γ_kM1_ws,  β_kM1_ws;
	...
	# [🐯 ] cost coefficients
	c_f = 1, c_x = 0, c_g = 0, c_p = 0,
	# [💁 ] initial condition coefficients
	ι_f = 1, ι_x = 0, ι_g = 0, ι_p = 0,
  ...
  maxCutCount=1e6, # number of lazy cuts when we use lazy constraints for modeling G = L_cholesky*L_cholesky
	...
	elseif impose_pattern_interpolation == :on
		...
		# alternative
		idx_set_λ = [i_j_idx(⋆, k-1); i_j_idx(⋆, k); i_j_idx(k-1, k)]
	...
	# β_kM1 * tr G C_{k-1, *} = tr G (C_{k,*} - η_2_input*D_{k,k-1})

	@constraint(Ada_PEP_model, β_kM1 * tr(G*C_mat(k-1, ⋆, 𝐠)) == tr(G * (C_mat(k, ⋆, 𝐠) - η_2_input*D_mat(k, k-1, 𝐠))) )
	...
	if (termination_status(Ada_PEP_model) == LOCALLY_SOLVED || termination_status(Ada_PEP_model) == SLOW_PROGRESS || termination_status(Ada_PEP_model) == OPTIMAL)
		...
		q_input = μ/L
		if η_2_input == 1
			@info "NCG-PEP Theory PRP ( f_{k+1} - f_⋆ ) / ( f_k - f_⋆ ) = $((1 - (μ/L)^2)^2/(1+(μ/L)^2)^2)"
		elseif η_2_input == 0
		    FR_contraction_fact = FR_contraction_factor_gen(c_0_input_initial, μ, L)
			@info "NCG-PEP Theory FR ( f_{k+1} - f_⋆ ) / ( f_k - f_⋆ ) = $(FR_contraction_fact)"
		end
		@info "NCG-PEP Numerical ( f_{k+1} - f_⋆ ) / ( f_k - f_⋆ )= $(t_opt)"
...
function SDP_with_fixed_stepsize_ratio(
	...
	@constraint(model_primal_PEP_with_known_stepsizes, hypographCon, t <= (Ft'*a_vec(⋆,k, 𝐟)))
...
function Dual_SDP_with_fixed_stepsize_ratio(
	...
	@constraint(model_dual_PEP_with_known_stepsizes, sum(λ[i_j_λ]*a_vec(i_j_λ.i,i_j_λ.j,𝐟) for i_j_λ in idx_set_λ) - a_vec(-1,k,𝐟) + ν*a_vec(-1,k-1,𝐟)  .== 0)
	...
	if impose_pattern == :on
	  ...
	  # idx_set_λ = [i_j_idx(⋆, k-1); i_j_idx(⋆, k); i_j_idx(k-1, k);  i_j_idx(k, k-1); i_j_idx( k-1, ⋆); i_j_idx( k, ⋆)]
	  # alternative
	   fix(λ[i_j_idx(k, k-1)], 0.0; force = true)
	   fix(λ[i_j_idx( k-1, ⋆)], 0.0; force = true)
	   fix(λ[i_j_idx(k, ⋆)], 0.0; force = true)

	   ϵ_test = sqrt(1-(1/c_0_input_initial))
	   q_ϵ = ((μ/L)*(1-ϵ_test))/(1+ϵ_test)
	   # fix(λ[i_j_idx(k-1, k)], (1-q_ϵ)/(1+q_ϵ); force = true)
	   # fix(λ[i_j_idx( ⋆, k-1)], (2*q_ϵ*(1-q_ϵ))/(1+q_ϵ)^2; force = true)
	   # fix(λ[i_j_idx(⋆, k)], 2*q_ϵ/(1+q_ϵ); force = true)
	   # fix(ρ, (γ_kM1^2*((L*(1 + ϵ_test)) + ((-1 + ϵ_test)*μ)))/(4*ϵ_test); force = true)
	   # fix(τ[1],  -((γ_kM1*(L*(1 +ϵ_test) + (-1 +ϵ_test)*μ))/(ϵ_test*(L + L*ϵ_test + μ -ϵ_test*μ))); force = true)
	   fix(τ[2], 0.0; force = true)
	   fix(τ[3], 0.0; force = true)
	   fix(τ[4], 0.0; force = true)
